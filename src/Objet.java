...
public abstract class Objet{
	...
	// Pourquoi la m�thode est abstraite si la classe l'est d�j�? Je la mets en commentaire en attendant - Reda
	...
	public LinkedList<Point> IntersectList(Objet poly2){
		...
		for(int i= 0; i<= this.npoints -1; i++){
			for(int j = 0; j<= poly2.npoints -1 ; j++){
				if(i!=this.npoints-1){ //Pour ne pas d�passer la taille du tableau
					if(j!= poly2.npoints-1){
	...
	public static int[] tri (LinkedList<Objet> liste){ 
		...
		//Methode du tri par insertion, plut�t rapide pour des petits effectifs ou des effectifs presque tri�s
